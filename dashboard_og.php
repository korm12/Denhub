...
<div id="main">
  <button onclick="openNav()" class="openbtn btn btn-md btn-outline-info">☰</button>  
    <div class="container shad">
        <h1 class="text-center title">On Going Appointments</h1>
            ...
            <tbody>
              <?php
               ...
               while($customer_information=$result->fetch_assoc()) {      
                  ...
                  echo "<td> 
                  <a class='btn btn-sm btn-outline-primary mt-1' href='update.php?id={$customer_information['id']}&s={$done}'>Done</a>
                  <a class='btn btn-sm btn-outline-danger mt-1' href='update.php?id={$customer_information['id']}&s={$cancel}'> Cancel</a>
                  <td>";
                  echo "</tr>";      
               }
                if(isset($connection)) {
